...
@[grind] def matchActionPattern (pattern : String) (action : String) : Bool :=
...
@[grind] def stmtGrantsAction (s : Statement) (action : String) : Bool :=
...
@[grind] def actionMatches (stmt : Statement) (action : String) : Bool :=
  stmtGrantsAction stmt action

@[grind] def resourceMatches (stmt : Statement) (resource : String) : Bool :=
...
@[grind] def stmtMatches (stmt : Statement) (req : Request) : Bool :=
  actionMatches stmt req.action && resourceMatches stmt req.resource

@[grind] def Statement.condBlocks (s : Statement) : CondBlocks :=
  (decodeCondBlocks s.condition).1

@[grind] def allows (p : Policy) (req : Request) (ctx : CondContext) : Bool :=
